...
#include <opencv2/video.hpp> // needed for background subtractor 
#include <stddef.h>
#include <string>
...
void Scene3DRenderer::processForeground(
		Camera* camera)
{
	Mat hsv_image,foreground, background, tmp;
	vector<Mat> channels, bg_channels;
	...
	// Get background
	bg_channels = camera->getBgHsvChannels();

	//Background subtraction H // Hue
	absdiff(channels[0], bg_channels.at(0), tmp);
	threshold(tmp, foreground, m_h_threshold, 255, CV_THRESH_BINARY + CV_THRESH_OTSU); // CV_THRESH_OTSU for adaptive thresholding

	// Background subtraction S // Saturation

	absdiff(channels[1], bg_channels.at(1), tmp);
	threshold(tmp, background, m_s_threshold, 255, CV_THRESH_BINARY + CV_THRESH_OTSU); //  + CV_THRESH_OTSU
	bitwise_or(foreground, background, foreground);

	// Background subtraction V // intensity
	absdiff(channels[2], bg_channels.at(2), tmp);
	threshold(tmp, background, m_v_threshold, 255, CV_THRESH_BINARY + CV_THRESH_OTSU);
	bitwise_or(foreground, background, foreground); //Using or - or gives most compleet picture atm

	//// POSTPROCESSING 
	// OPEN = First erode, then dilate
	morphologyEx(foreground, foreground, MORPH_OPEN, Mat(), Point(-1, -1), 2);
