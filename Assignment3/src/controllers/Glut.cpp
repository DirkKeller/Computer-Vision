	...
	int v)
{
	...
	if (scene3d.getCurrentFrame() != scene3d.getPreviousFrame())
	{
		...
		scene3d.getReconstructor().rectifyLabels();
	...
	// ----------------------------------------------- OUR CODE: set reference color histogram ---------------------------------------------
	if (scene3d.getCurrentFrame() <= 1) {
		std::vector<cv::Mat> color_hists = scene3d.getReconstructor().getColorHist();
		scene3d.getReconstructor().setReferenceColorHist(color_hists);
	}
	// -------------------------------------------------------------------------------------------------------------------------------------
...
void Glut::drawVoxels()
{
	...
	// Get labels and voxels
	Mat labels = m_Glut->getScene3d().getReconstructor().getClusterLabels();
	vector<Reconstructor::Voxel*> voxels = m_Glut->getScene3d().getReconstructor().getVisibleVoxels();

	// Get the transform vector
	std::vector<std::vector<int>> transform_vector = m_Glut->getScene3d().getReconstructor().getTransformVector();
	vector<int> current_transform = transform_vector.back();

	glBegin(GL_POINTS);

	int v;
	for (v = 0; (int)v < voxels.size(); v++)
	{
		Scalar bgr_value = voxels[v]->color / 255; //normalize to GLfloat value

		// Color the voxel according to the rectified labels
		if (labels.at<int>(v) == current_transform[0]) glColor3f((GLfloat)bgr_value.val[2] / 4, (GLfloat)bgr_value.val[0], (GLfloat)bgr_value.val[1]);
		else if (labels.at<int>(v) == current_transform[1]) glColor3f((GLfloat)bgr_value.val[2], (GLfloat)bgr_value.val[0] / 8, (GLfloat)bgr_value.val[1] / 2);
		else if (labels.at<int>(v) == current_transform[2]) glColor3f((GLfloat)bgr_value.val[2], (GLfloat)bgr_value.val[0] / 2, (GLfloat)bgr_value.val[1] / 8);
		else if (labels.at<int>(v) == current_transform[3]) glColor3f((GLfloat)bgr_value.val[2] / 8, (GLfloat)bgr_value.val[0], (GLfloat)bgr_value.val[1] / 2);

		// Draw the voxels accoring to the coordinates
...
/**
 * Track the motion of the moving cluster
 */
void Glut::motionTracking()
{
	glPushMatrix();
...
std::vector<std::vector<cv::Point2f>> centers = m_Glut->getScene3d().getReconstructor().getClusterCenters();
std::vector<std::vector<int>> transform_vector = m_Glut->getScene3d().getReconstructor().getTransformVector();

// Color scheme used for motion tracking
Scalar colorTab[] =
{
	Scalar(64, 255, 255),
	Scalar(255, 32, 126),
...
// Start motion tracking (after 1st frame)
if (m_Glut->getScene3d().getCurrentFrame() > 1)
{
	int c, l;
	for (c = 0; c < (int)centers[0].size(); ++c) // for each individual
	{
		...
		// Enable continious lines
		glBegin(GL_LINE_STRIP); 
		// Color the the lines based on the color scheme of the individual
		glColor4f((GLfloat)colorTab[c][0] / 255, (GLfloat)colorTab[c][1] / 255, (GLfloat)colorTab[c][2] / 255, 0.3f);
#pragma omp parallel for schedule(static) private(l) shared(transform_vector, centers)
		for (l = 0; l < (int)centers.size(); ++l) // for each center
		{
			// Connect each center position of the individual with rectified labels across frames
			int truePos = transform_vector[l][c];
			if (transform_vector[l][c] == 0) glVertex2f((GLfloat)centers[l][truePos].x, (GLfloat)centers[l][truePos].y);
			else if (transform_vector[l][1] == 1) glVertex2f((GLfloat)centers[l][truePos].x, (GLfloat)centers[l][truePos].y);
			else if (transform_vector[l][2] == 2) glVertex2f((GLfloat)centers[l][truePos].x, (GLfloat)centers[l][truePos].y);
			else if (transform_vector[l][3] == 3) glVertex2f((GLfloat)centers[l][truePos].x, (GLfloat)centers[l][truePos].y);
		}
...
void Glut::resetCenters(int type)
{
	
	if (type == 1) 
	{
		std::vector<std::vector<cv::Point2f>> cluster_centers = m_Glut->getScene3d().getReconstructor().getClusterCenters();
		cluster_centers.pop_back();
		m_Glut->getScene3d().getReconstructor().setClusterCenters(cluster_centers);
	}
	else if (type == 2)
	{
		std::vector<std::vector<cv::Point2f>> cluster_centers;
		m_Glut->getScene3d().getReconstructor().setClusterCenters(cluster_centers);
	}
}
} /* namespace nl_uu_science_gmt */
